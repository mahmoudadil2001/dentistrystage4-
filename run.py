...
# تحميل التنسيق (اختياري - أنشئ styles.css بنفس مجلد السكربت)
...
def show_chat_page():
    ...
    for u in online_users:
        if u.profile_picture:
            st.sidebar.image(os.path.join(UPLOAD_DIR, u.profile_picture), width=35, caption=u.username)
        else:
...
# إعداد الصفحة
st.set_page_config(page_title="غرفة الدردشة", layout="wide")
st.markdown("""
<style>
body, .main > div {
...
.message-image {
    max-width: 300px; max-height: 200px; border-radius: 6px; margin-top: 6px;
}
</style>
